...
// ─── Tipos ────────────────────────────────────────────────────────────────────
type PsychTool = 'indicadores' | 'escalas' | 'trauma' | 'traduccion';
...
type PsychToolDef = {
  ...
  requiresNotes?: boolean;         // textarea de notas clínicas
  captureIndicators?: boolean;     // guarda res.indicadores para "Análisis de Trauma"
  usesIndicators?: boolean;        // envía los indicadores previamente extraídos
};

const TOOLS: Record<PsychTool, PsychToolDef> = {
  indicadores: {
    icon: '🔍',
    label: 'Extraer Indicadores',
    endpoint: '/psychological-tools/indicators/extract',
    usesTranscription: true,
    captureIndicators: true,
  },
  escalas: {
    icon: '📊',
    label: 'Escalas de Riesgo',
    endpoint: '/psychological-tools/risk-scales/prefill',
    usesTranscription: true,
    requiresTranscription: true,
  },
  trauma: {
    icon: '🧠',
    label: 'Análisis de Trauma',
    endpoint: '/psychological-tools/trauma/analyze',
    usesIndicators: true,
  },
  traduccion: {
    icon: '🔤',
    label: 'Traducción Clínica',
    endpoint: '/psychological-tools/clinical-translator/translate',
    requiresNotes: true,
  },
};

const ACCENT = '#7C3AED'; // violeta — identidad visual de la disciplina

function PsychologicalToolsContent() {
  const params = useSearchParams();
  const caseId      = params.get('caseId') || '';
  const tool        = (params.get('tool') || 'indicadores') as PsychTool;
  ...
  const [activeTool,       setActiveTool]       = useState<PsychTool>(tool);
  const [transcriptionId,  setTranscriptionId]  = useState('');
  const [notesText,        setNotesText]        = useState('');
  const [traumaIndicators, setTraumaIndicators] = useState<string[]>([]);
  ...
  const handleAnalyze = async () => {
    ...
    if (def.requiresNotes && !notesText.trim()) {
      setError('Escribí las notas clínicas a traducir antes de ejecutar la herramienta.');
      return;
    }
    ...
    if (def.usesIndicators) body.indicadores = traumaIndicators;
    if (def.requiresNotes) body.notesText = notesText;
    ...
    try {
      ...
      if (def.captureIndicators && Array.isArray(res?.indicadores)) {
        setTraumaIndicators(res.indicadores);
      }
  ...
  return (
    <div style={{ maxWidth: '1300px', margin: '0 auto' }}>
      <BreadcrumbBar caseData={caseData} areaLabel="Herramientas Psicológicas" />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div style={{ backgroundColor: 'var(--card)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {(Object.entries(TOOLS) as [PsychTool, PsychToolDef][]).map(([key, t]) => (
                <button
                  key={key}
                  type="button"
          ...
          <div style={{ backgroundColor: 'var(--card)', padding: '1.5rem', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
            ...
            {def.usesTranscription && (
              <div style={{ marginBottom: '1rem' }}>
                <label htmlFor="psic-transcription-id" style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, color: 'var(--grafito)', marginBottom: '0.375rem' }}>
                  ID de transcripción {def.requiresTranscription ? '(obligatorio)' : '(opcional)'}
                </label>
                <input
                  id="psic-transcription-id"
                  type="text"
            ...
            {def.requiresNotes && (
              <div style={{ marginBottom: '1rem' }}>
                <label htmlFor="psic-notes" style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, color: 'var(--grafito)', marginBottom: '0.375rem' }}>
                ...
                <textarea
                  ...
                  placeholder="Pegá las notas clínicas a traducir a lenguaje forense..."
                  style={{ width: '100%', padding: '0.625rem', borderRadius: 'var(--radius)', border: '1px solid var(--border)', backgroundColor: 'var(--papel)', fontSize: '0.8rem', color: 'var(--grafito)', fontFamily: 'inherit', resize: 'vertical' }}
                />
              </div>
            )}

            {def.usesIndicators && (
              <div style={{ marginBottom: '1rem', fontSize: '0.75rem', color: 'var(--grafito)', backgroundColor: 'var(--papel)', padding: '0.625rem', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
                {traumaIndicators.length > 0
                  ? `🧠 Indicadores a analizar: ${traumaIndicators.length} extraídos. Ejecutá "Extraer Indicadores" para renovarlos.`
                  : '⚠️ Todavía no extrajiste indicadores. Ejecutá "Extraer Indicadores" para un análisis sustantivo (o el análisis se genera sin indicadores).'}
              </div>
            )}
...
export default function PsychologicalPage() {
  return (
    <Suspense fallback={<LoadingState label="herramientas psicológicas" />}>
      <PsychologicalToolsContent />
    </Suspense>
  );
}
...
function formatResult(res: any): string {
  ...
  if (res && typeof res === 'object') {
    for (const key of ['analisisCompleto', 'forensicTranslation', 'ollamaAnalysis', 'response', 'message']) {
